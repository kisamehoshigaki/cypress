export default {
    accountSettingsLanguageSelect: '#_com_liferay_my_account_web_portlet_MyAccountPortlet_languageId',
    homePage: {
      helloWorldContainer: '#fragment-0-maqp',
      welcomeContainer: '#fragment-0-btao'
    },
    login: {
      emailInput: '#_com_liferay_login_web_portlet_LoginPortlet_login',
      loginButton: '.button-holder button',
      loginLink: 'span.sign-in a',
      passwordInput: '#_com_liferay_login_web_portlet_LoginPortlet_password'
    },
    primaryButton: 'button.btn.btn-primary',
    simulator: {
      closeSimulator: '#ored____',
      height: '#_com_liferay_product_navigation_simulation_web_portlet_SimulationPortlet_height',
      openSimulation: '#_com_liferay_product_navigation_simulation_web_portlet_SimulationPortlet_simulationToggleId',
      sidebar: '.product-menu.sidebar.sidebar-inverse',
      width: '#_com_liferay_product_navigation_simulation_web_portlet_SimulationPortlet_width'
    },
    userAvatar: '.user-avatar-link'
  };
  